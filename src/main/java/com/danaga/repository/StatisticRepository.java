...
public interface StatisticRepository extends JpaRepository<Statistic, String>{
	...
	// 환불대기+환불완료+취소 건수
	@Query(value = "select ifnull(count(create_date),0) from orders where date_format(create_date,'%Y%m') = :findMonth  and (statement = '환불대기중' or statement = '환불완료' or statement = '취소')", nativeQuery = true)
